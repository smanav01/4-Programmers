...
package datamanagement;
	...
	public class Unit implements IUnit //class is defined as class unit 
	{
	private String uc;
	private String UN;
	private float co2;
	...
	float f5, int i1, int i2, int i3, StudentUnitRecordList rl)
	 {
		...
		co3 = f4;
		this.co5 = f5;
		this.setAssessmentWeights(i1, i2, i3);//set values may be different because of float values
	...
	@Override
	public void setAssessmentWeights(int a1, int a2, int ex)//assessment values set as a integer
	 {
		if (a1 < 0 || a1 > 100 ||
			a2 < 0 || a2 > 100 ||
			ex < 0 || ex > 100 ) 
	...
	private void setCutoffs( float ps, float cr, float di, float hd, float ae) //cutoffs values will remain in float.
		{
		if (ps < 0 || ps > 100 ||
			cr < 0 || cr > 100 ||
			di < 0 || di > 100 ||
